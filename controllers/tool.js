...
async function alarmRemainderPayment2Parent(
    res,
    accessToken,
    zaloUserId,
    classId,
...
async function listStudentRemainderPayment(classId, currentTerm, studentInfoColl) {
    // Lay danh sach hoc sinh co tien thua dot x lop y
    const pipeline = [
        {
            $match: {
                $and: [
...
async function findZaloIdFromStudentId(zaloColl, zaloStudentId, role) {
    const cursor = zaloColl.find(
        { 'students.zaloStudentId': parseInt(zaloStudentId), 'students.role': role },
